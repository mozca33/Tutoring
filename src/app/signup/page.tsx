    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-background px-6">
        ...
        <div className="w-full max-w-md bg-surface p-8 rounded-xl shadow-sm border border-border text-center space-y-4">
          ...
          <p className="text-sm text-muted">Enviamos um link de confirmação para <strong className="text-foreground">{email}</strong>. Abra para ativar sua conta.</p>
          <Link href="/login" className="inline-block w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-lg font-medium transition-colors">Ir para o login</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen flex items-center justify-center bg-background px-6">
      ...
      <form onSubmit={onSubmit} className="w-full max-w-md bg-surface p-8 rounded-xl shadow-sm border border-border space-y-4">
        <div>
          ...
          <p className="text-sm text-muted mt-1">É aluno? Sua conta é criada pelo seu professor — peça um convite por e-mail.</p>
        </div>
