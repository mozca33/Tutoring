...
export default function ResetPasswordPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (error) { setLoading(false); return setError("Não foi possível definir a senha. Abra novamente o link e tente de novo."); }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-background px-6">
      ...
      <div className="w-full max-w-md bg-surface p-8 rounded-xl shadow-sm border border-border space-y-4">
        <h1 className="text-2xl font-semibold text-foreground">Defina sua senha</h1>

        {status === "checking" && (
          <p className="text-sm text-muted flex items-center gap-2"><Loader2 size={16} className="animate-spin" /> Validando o link…</p>
        )}

        {status === "invalid" && (
          <p className="text-sm text-red-600 dark:text-red-400">
            Link inválido ou expirado. Peça ao seu professor para gerar um novo link de acesso.
          </p>
        )}
